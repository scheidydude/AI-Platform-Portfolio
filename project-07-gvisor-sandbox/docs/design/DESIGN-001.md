...
ContainerRunner.run_task_isolated()                        [existing]
        ...
        │  - --memory, --cpus                                 [FR-2]
        │  - --network none, or allowlist via egress proxy    [FR-3]
        ▼
...
WorkerResult{task_id, success, result, error, duration_s, cpu_seconds,
             + optional stdout, stderr, exit_code}            [FR-4, ADR-002]
        ▼
Existing consumers unchanged: orchestrator.py, remote/worker_server.py,
...
| 3 — Network Policy | FR-3 | `--network none` by default; explicit domain allowlist via existing Traefik/proxy |
