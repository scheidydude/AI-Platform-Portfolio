...
**Current state:** Phases 1–2 complete. `runsc` installed and verified on the NucBox; syscall-interception isolation proven; `ContainerRunner` hardened with `--runtime`/`--memory`/`--cpus` config and `WorkerResult` extended additively. Committed to the **Orchid repo's `p07-gvisor-hardening` branch** (`0eaac75`) — not merged to `main`, that's David's call, not part of this task.
...
**Exact next action:** Phase 3 — network policy. Add `--network none` by default to `ContainerRunner._build_docker_command()`, plus an allowlist mode routed through the confirmed Traefik config (still not confirmed which of the two candidates found in Phase 0 governs sandbox egress — resolve that first). See `task_plan.md` Phase 3 and SRS-001 FR-3.
